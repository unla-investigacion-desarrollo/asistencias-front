const edificios = [
    'Edificio Jose Hernandez',
    'Edificio Arturo Jauretche',
    'Edificio Juana Manso',
    'Edificio Leopoldo Marechal',
    'Edificio Manuel Ugarte',
    'Edificio Raul Scalabrini Ortiz',
    'Edificio Hector Oesterheld',
    'Edificio Hernandez Arregui',
    'Edificio Homero Manzi',
    'Edificio Juana Azurduy',
    'Edificio Leonardo Werthein',
    'Edificio Lisandro de la Torre',
    'Edificio Lola Mora',
    'Edificio Macedonio Fernandez',
    'Edificio Maria Remedios del Valle',
    'Edificio Paco Urondo',
    'Edificio Rodolfo Ortega Pena',
    'Edificio Irma Laciar de Carrica',
    'Cine Universitario Tita Merello',
    'Campo de Deportes Delfo Cabrera'
];

export {
    edificios
};
